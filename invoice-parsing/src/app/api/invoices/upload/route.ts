...
import PDFProcessor from '@/app/api/invoices/services/pdfProcessor';
import LLMExtractor from '@/app/api/invoices/services/llmExtractor';
// import ValidationService from '@/app/api/invoices/services/validator';

const pdfProcessorObj = new PDFProcessor();
...
export const POST = auth(async (req) => {
  ...
  for (const file of files) {
    ...
    // Process PDF -> images
    const pdfData = await pdfProcessorObj.processPDF(filePath);
    ...
    // LLM extraction
    const extraction = await llmExtractor.retryExtraction(schema, pdfData.base64Images);
